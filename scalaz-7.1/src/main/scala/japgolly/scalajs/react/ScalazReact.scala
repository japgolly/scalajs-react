...
object ScalazReact {
  ...
  final class SzRExt_Callback[A](private val _c: () => A) extends AnyVal {
  ...
  @inline implicit def callbackToSzRExt_Callback[A](c: CallbackTo[A]) =
    new SzRExt_Callback(c.toScalaFunction)

  implicit final class SzRExt_CallbackConvertable[M[_], A](private val m: M[A]) extends AnyVal {
  ...
  @inline def StateAndCallbacks[S](s: S, cb: Callback = Callback.empty) = new StateAndCallbacks[S](s, cb)
  ...
  final class SzRExt_CompStateAccessOps[C, S](private val $: C) extends AnyVal {
