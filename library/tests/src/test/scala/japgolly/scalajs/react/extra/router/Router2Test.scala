...
object Router2TestRoutes {
  ...
  object MyPage2 {
    ...
    val isUserLoggedInCB = CallbackTo(isUserLoggedIn)
    var secret = "apples"
    ...
    val config = RouterConfigDsl[MyPage2].buildConfig { dsl =>
      ...
      val privatePages12 = (emptyRule
        ...
        .addConditionWithFallback(isUserLoggedInCB, redirectToPage(PublicHome)(SetRouteVia.HistoryPush))

      val privatePage3 = (emptyRule
        | staticRoute("private-3", PrivatePage3) ~> render(<.h1("Private #3"))
        )
        .addConditionWithFallback(isUserLoggedInCB, redirectToPage(AccessDenied)(SetRouteVia.HistoryReplace))

      val privatePage4 = (emptyRule
        | staticRoute("private-4", PrivatePage4) ~> render(<.h1("Private #4"))
        )
        .addConditionWithFallbackBy(_ => isUserLoggedInCB, redirectToPage(AccessDenied)(SetRouteVia.HistoryReplace))
...
object Router2Test extends TestSuite {
  ...
  override val tests = Tests {
    ...
    val r = LegacyReactTestUtils.renderIntoDocument(router())
