...
* Introduction of `ScalaJsReactConfig` which allows a few select global settings.
  Inside there's also `ScalaJsReactConfig.DevOnly` which only affects `fastOptJS` and is removed at compile-time from `fullOptJS`.
...
* Component names can now be omitted and generated automatically.
  If you'd like to use this immediately with all of your existing components there's a migration at the end of this page.
  ...
  would be automatically named `"example.Blah.TopLevel"`.

  By default, `"Comp"` and `"Component"` are trimmed from names. This behaviour can be overriden by calling
  `ScalaJsReactConfig.componentNameModifier*` methods.
  For example, you might want to call `ScalaJsReactConfig.componentNameModifierAppend(_.stripPrefix("com.github.example."))`
  to trim package names.
...
* The `modStateL` and `modStateOptionL` methods provided by `MonocleReact` now accept all kinds of appropriate optics instead of just lenses
