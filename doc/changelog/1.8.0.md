...
* Tweaked `renderBackend` algorithm for Scala 3 compatibility so that the names of your render method's parameters
  is considered before their types. Basically if you have any parameters named...
  * `p` or `props`, `renderBackend` will provide an instance of the current `Props` (regardless of whether the types line up or not)
  * `s` or `state`, `renderBackend` will provide an instance of the current `State` (regardless of whether the types line up or not)
