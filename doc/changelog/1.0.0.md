...
1. There is now an underlying, generic representation of components. Language-agnostic. Scala components (created with `ScalaComponent.build`, previously `ReactComponentB`) are no longer a special case and have no special priveliges in the library.
...
- **Proper representation of non-Scala components.**
  ...
  - `JsComponent` - It's now a one-liner to use community components written in pure JS.
    No more hacks!
  ...
  - `ScalaComponent` - Create React components in Scala using the functional builder with an optional backend.
    ...
    Where as there used to be `ReactComponentB[Props]…` before, it's now `ScalaComponent.build[Props]…`.
...
* **Virtual DOM major revision**
  * Under-the-hood, the types have been rewritten and simplified, and is now easier to work and maintain.
    This no longer bears any resemblence to Scalatags.
    Scalatags was however, *tremedously* helpful in this journey so if you have a chance,
...
* **Simple types for various purposes**
  ...
  Also: components no longer track DOM types. In cases where accessomg a component's DOM is desirable, add a method to the backend (or externally) that casts it.
  Example:
  ```scala
...
* Reduction: `ExternalVar` and `ReusableVar` have been consolidated into a single class called `StateSnapshot`.
  ...
  * Users have one type to learn, and no longer have to learn subtlties between two very similar types.
...
* Reduction: `ReusableFn`, `ReusabaleVal`, and `ReusabaleVal2` have all been consolidated into a single class called `Reusable`.
  ...
  (The `A ~=> B` alias now relfects this so you won't need any change where the alias is used.)
...
<br>Then run:
```sh
curl -s https://raw.githubusercontent.com/japgolly/scalajs-react/topic/neo/doc/changelog/1.0.0-migrate.sh | bash
```

Alternatively you can open up [1.0.0-migrate.sh](changelog/1.0.0-migrate.sh) in your browser, inspect it,
...
* If you're using any of these libraies, upgrade them to:
  * Monocle 1.4.0
  * ScalaCSS x.y.z
  * Test State 2.1.2
* VDOM changes
