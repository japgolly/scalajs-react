...
object ReusableFn {
  ...
  @inline def apply[A: Reusable, Y, Z](f: (A, Y) => Z): A ~=> (Y ~=> Z) =
    new Fn2(f)

  @inline def apply[A: Reusable, B: Reusable, Y, Z](f: (A, B, Y) => Z): A ~=> (B ~=> (Y ~=> Z)) =
    new Fn3(f)

  @inline def apply[A: Reusable, B: Reusable, C: Reusable, Y, Z](f: (A, B, C, Y) => Z): A ~=> (B ~=> (C ~=> (Y ~=> Z))) =
    new Fn4(f)

  @inline def apply[A: Reusable, B: Reusable, C: Reusable, D: Reusable, Y, Z](f: (A, B, C, D, Y) => Z): A ~=> (B ~=> (C ~=> (D ~=> (Y ~=> Z)))) =
    new Fn5(f)

  @inline def apply[A: Reusable, B: Reusable, C: Reusable, D: Reusable, E: Reusable, Y, Z](f: (A, B, C, D, E, Y) => Z): A ~=> (B ~=> (C ~=> (D ~=> (E ~=> (Y ~=> Z))))) =
    new Fn6(f)
  ...
  implicit def reusability[A, B]: Reusable[ReusableFn[A, B]] =
    Reusable.fn((x, y) => (x eq y) || x.reusable.applyOrElse(y, (_: ReusableFn[A, B]) => false))
  ...
  private class Fn2[A: Reusable, Y, Z](val f: (A, Y) => Z) extends ReusableFn[A, Y ~=> Z] {
    override def apply(a: A) = new Cur1(a, f)
    override private[extra] def reusable = { case x: Fn2[A, Y, Z] => f eq x.f }
  }

  private class Fn3[A: Reusable, B: Reusable, Y, Z](val f: (A, B, Y) => Z) extends ReusableFn[A, B ~=> (Y ~=> Z)] {
    private val c2 = cur2(f)
    override def apply(a: A) = new Cur1(a, c2)
    override private[extra] def reusable = { case x: Fn3[A, B, Y, Z] => f eq x.f }
  }

  private class Fn4[A: Reusable, B: Reusable, C: Reusable, Y, Z](val f: (A, B, C, Y) => Z) extends ReusableFn[A, B ~=> (C ~=> (Y ~=> Z))] {
  ...
  private class Fn5[A: Reusable, B: Reusable, C: Reusable, D: Reusable, Y, Z](val f: (A, B, C, D, Y) => Z) extends ReusableFn[A, B ~=> (C ~=> (D ~=> (Y ~=> Z)))] {
  ...
  private class Fn6[A: Reusable, B: Reusable, C: Reusable, D: Reusable, E: Reusable, Y, Z](val f: (A, B, C, D, E, Y) => Z) extends ReusableFn[A, B ~=> (C ~=> (D ~=> (E ~=> (Y ~=> Z))))] {
  ...
  @inline private def cur2[A:Reusable, B:Reusable,                                     Y, Z](f: (A,B,      Y) => Z): (A,B      ) => (Y ~=> Z) = new Cur2(_,_,      f)
  @inline private def cur3[A:Reusable, B:Reusable, C:Reusable,                         Y, Z](f: (A,B,C,    Y) => Z): (A,B,C    ) => (Y ~=> Z) = new Cur3(_,_,_,    f)
  @inline private def cur4[A:Reusable, B:Reusable, C:Reusable, D:Reusable,             Y, Z](f: (A,B,C,D,  Y) => Z): (A,B,C,D  ) => (Y ~=> Z) = new Cur4(_,_,_,_,  f)
  @inline private def cur5[A:Reusable, B:Reusable, C:Reusable, D:Reusable, E:Reusable, Y, Z](f: (A,B,C,D,E,Y) => Z): (A,B,C,D,E) => (Y ~=> Z) = new Cur5(_,_,_,_,_,f)

  private class Cur1[A: Reusable, Y, Z](val a: A, val f: (A, Y) => Z) extends ReusableFn[Y, Z] {
    override def apply(y: Y): Z = f(a, y)
    override private[extra] def reusable = { case x: Cur1[A, Y, Z] => (f eq x.f) && (a ~=~ x.a) }
  }

  private class Cur2[A: Reusable, B: Reusable, Y, Z](val a: A, val b: B, val f: (A, B, Y) => Z) extends ReusableFn[Y, Z] {
    override def apply(y: Y): Z = f(a, b, y)
    override private[extra] def reusable = { case x: Cur2[A, B, Y, Z] => (f eq x.f) && (a ~=~ x.a) && (b ~=~ x.b) }
  }

  private class Cur3[A: Reusable, B: Reusable, C: Reusable, Y, Z](val a: A, val b: B, val c: C, val f: (A, B, C, Y) => Z) extends ReusableFn[Y, Z] {
    override def apply(y: Y): Z = f(a, b, c, y)
    override private[extra] def reusable = { case x: Cur3[A, B, C, Y, Z] => (f eq x.f) && (a ~=~ x.a) && (b ~=~ x.b) && (c ~=~ x.c) }
  }

  private class Cur4[A: Reusable, B: Reusable, C: Reusable, D: Reusable, Y, Z](val a: A, val b: B, val c: C, val d: D, val f: (A, B, C, D, Y) => Z) extends ReusableFn[Y, Z] {
  ...
  private class Cur5[A: Reusable, B: Reusable, C: Reusable, D: Reusable, E: Reusable, Y, Z](val a: A, val b: B, val c: C, val d: D, val e: E, val f: (A, B, C, D, E, Y) => Z) extends ReusableFn[Y, Z] {
