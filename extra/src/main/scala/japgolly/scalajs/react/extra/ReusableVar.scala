...
object ReusableVar {
  ...
  def s$[S: Reusability](s: S, $: CompState.WriteAccess[S]): ReusableVar[S] =
    new ReusableVar[S](s, ReusableFn($).setState)

  def state[S: Reusability]($: CompState.AccessRD[S]): ReusableVar[S] =
