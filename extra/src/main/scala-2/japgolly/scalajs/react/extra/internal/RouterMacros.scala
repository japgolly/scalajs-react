...
object RouterMacros {
  ...
  trait ForRouteB[A] {
    ...
    def caseClass[B]: RouteB[B] =
      macro RouterMacros.quietCaseClassBuilder[B]

    /** Same as [[caseClass]] except the code generated by the macro is printed to stdout. */
    def caseClassDebug[B]: RouteB[B] =
      macro RouterMacros.debugCaseClassBuilder[B]
  }
}
...
class RouterMacros (val c: Context) extends MacroUtils {
  ...
  def quietCaseClassBuilder[T: c.WeakTypeTag]: c.Expr[RouteB[T]] = implCaseClass[RouteB, T](false)
  def debugCaseClassBuilder[T: c.WeakTypeTag]: c.Expr[RouteB[T]] = implCaseClass[RouteB, T](true)
